...
describe Puppet::Type.type(:f5_monitor).provider(:https) do
  let(:resource) do
    Puppet::Type.type(:f5_monitor).new(
      ...
      password:           '$M$qM$p1gfRVYxrpzfxDN+WBVqLw==',
      receive_string:     'nou',
      reverse:            'disabled',
  ...
  describe 'instances' do
    it 'gets a response from the api' do
      result = nil
      VCR.use_cassette('f5_monitor/https/instances') do
  ...
  describe 'create' do
    it 'gets a response from the api' do
      result = nil
      VCR.use_cassette('f5_monitor/https/create') do
  ...
  describe 'flush' do
    it 'gets a response from the api' do
      result = nil
      VCR.use_cassette('f5_monitor/https/flush') do
        provider.class.prefetch({ '/Common/https' => resource})
  ...
  describe 'destroy' do
    it 'gets a response from the api' do
      result = nil
      VCR.use_cassette('f5_monitor/https/destroy') do
