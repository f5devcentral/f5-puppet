...
Puppet::Type.type(:f5_virtualserver).provide(:performance_l4, parent: Puppet::Provider::F5) do
  def self.instances
    instances = []
  ...
  def message(object)
    ...
    if message[:protocol_profile_client]
      if self.class.find_profile_type(message[:protocol_profile_client]) == "fastl4"
