...
Puppet::Type.type(:f5_monitor).provide(:ldap, parent: Puppet::Provider::F5) do
  ...
  def self.instances
    instances = []
    monitors = Puppet::Provider::F5.call_items('/mgmt/tm/ltm/monitor/ldap')
  ...
  def flush
    if @property_hash != {}
      result = Puppet::Provider::F5.put("/mgmt/tm/ltm/monitor/ldap/#{api_name}", message(@property_hash))
    end
    return result
  end
  ...
  def destroy
    result = Puppet::Provider::F5.delete("/mgmt/tm/ltm/monitor/ldap/#{api_name}")
    @property_hash.clear
