...
Puppet::Type.type(:f5_persistencedestaddr).provide(:rest, parent: Puppet::Provider::F5) do

  def self.instances
    instances = []
    addresses = Puppet::Provider::F5.call_items('/mgmt/tm/ltm/persistence/dest-addr')
  ...
  def flush
    if @property_hash != {}
      ...
      result = Puppet::Provider::F5.put("/mgmt/tm/ltm/persistence/dest-addr/#{full_path_uri}", message(resource))
    end
    return result
  end
  ...
  def create
    result = Puppet::Provider::F5.post("/mgmt/tm/ltm/persistence/dest-addr", message(resource))
  ...
  def destroy
    full_path_uri = resource[:name].gsub('/','~')
    result = Puppet::Provider::F5.delete("/mgmt/tm/ltm/persistence/dest-addr/#{full_path_uri}")
    @property_hash.clear
