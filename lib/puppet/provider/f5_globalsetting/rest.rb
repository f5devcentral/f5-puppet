...
Puppet::Type.type(:f5_globalsetting).provide(:rest, parent: Puppet::Provider::F5) do
  ...
  def self.instances
    instances = []
    settings = Puppet::Provider::F5.call('/mgmt/tm/sys/global-settings')

      instances << new(
        hostname:                 settings['hostname'],
        gui_setup:                settings['guiSetup'],
      )
    instances
  end
  ...
  def self.prefetch(resources)
   ...
   # we only have one setting entry, and it always exists
      if provider = settings.find { |setting| TRUE  }
  ...
  def flush
    if @property_hash != {}
      ...
      result = Puppet::Provider::F5.put("/mgmt/tm/sys/global-settings", message(flush_message))
    end
    return result
  end
  ...
  def create
    result = Puppet::Provider::F5.post("/mgmt/tm/sys/global-settings", message(resource))
  ...
  def destroy
    result = Puppet::Provider::F5.delete("/mgmt/tm/sys/global-settings")
    @property_hash.clear
