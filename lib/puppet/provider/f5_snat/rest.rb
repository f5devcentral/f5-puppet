...
Puppet::Type.type(:f5_snat).provide(:rest, parent: Puppet::Provider::F5) do
  ...
  def self.instances
    instances = []
    profiles = Puppet::Provider::F5.call_items('/mgmt/tm/ltm/snat')
...
#      :'fallback-host'          => :fallbackHost,
  ...
  def flush
    if @property_hash != {}
      ...
      result = Puppet::Provider::F5.put("/mgmt/tm/ltm/snat/#{full_path_uri}", message(resource))
    end
    return result
  end
  ...
  def create
    result = Puppet::Provider::F5.post("/mgmt/tm/ltm/snat", message(resource))
  ...
  def destroy
    full_path_uri = resource[:name].gsub('/','~')
    result = Puppet::Provider::F5.delete("/mgmt/tm/ltm/snat/#{full_path_uri}")
    @property_hash.clear
